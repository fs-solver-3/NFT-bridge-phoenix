defmodule BridgeServer.Minter do
  alias BridgeServer.Metaplex
  ...
  def mint(payer, metadata) do
    ...
    solana_network = Application.get_env(:bridge_server, :solana_network)
    ...
    [{:ok, mint_balance}, {:ok, token_balance}, {:ok, %{"blockhash" => blockhash}}] =
      Solana.RPC.send(client, tx_reqs)

    {:ok, associated_token} =
      Solana.SPL.AssociatedToken.find_address(Solana.pubkey!(mint), Solana.pubkey!(payer))
